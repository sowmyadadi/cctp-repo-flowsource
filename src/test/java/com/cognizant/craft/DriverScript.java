...
import com.cognizant.reportclient.plugins.clients.ReportClient;
...
public class DriverScript {
    ...
    public void driveTestExecution() {
        ...
        // Initialization of Test Iteration & WebDriver
        // Try Catch Handled for CCTP reporting
        startUp();
        try {
            initializeTestIterations();
            initializeWebDriver();
        } catch (Exception ex) {
            report.updateTestLog("DriverScript", ex.getLocalizedMessage(), ex, ex.getMessage(), Status.FAIL);
            CRAFTPlugin.updateTestCase(testParameters.getTestCaseUniqueId(), testParameters.getCurrentTestcase(),
                    "FAIL");
            throw ex;
        }

        // Initialization of Test Reports & Datatable
        // Try Catch Handled for CCTP reporting
        try {
    ...
    private void startUp() {
        ...
        // changes added by RAM on 14-Jun-2021
    ...
    private void initializeWebDriver() {
        ReportClient reportClient = ReportClient.getInstance();
        ...
        switch (testParameters.getExecutionMode()) {
            ...
            case LOCAL:
               ...
               // Below get the boolean which checks for argument -DCCTP.
                /*if (BrowserManager.getHaveArguments()) {
                    // Below get the value from ReporClient hash map
                    browserName= BrowserManager.getBrowserName();
                }*/
                if (reportClient.hasArguments()) {
                    // Below get the value from ReporClient hash map
                    browserName= reportClient.getOrDefault("browser", "CHROME");
                } else {
                   browserName = testParameters.getBrowser().toString();
                }
                webDriver = WebDriverFactory.getWebDriver(Browser.valueOf(browserName));
        ...
        // changes added by RAM on 14-Jun-2021
        /*re assigning driver after initialization*/
        setReportParameters();
    }
    ...
    private void initializeTestScript() {
        ...
        scriptHelper = new ScriptHelper(dataTable, report, driver, driverUtil, extentTest, reusableHandle);
        if (!isAPITest()) {
